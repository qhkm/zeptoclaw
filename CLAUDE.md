...
- Reasoning models served over OpenAI-compatible endpoints (DeepSeek, Qwen, and LiteLLM/Ollama fronting them) return their thinking in `reasoning_content` and may leave `content` null. `OpenAIResponseMessage` and `OpenAIStreamDelta` now parse it, `OpenAIChoice` parses `finish_reason`, and `resolve_assistant_text()` falls back to the reasoning when the model produced no content and no tool calls — logging a warning naming `finish_reason` (`length` means the budget went to thinking, so raise `max_tokens`). A tool-call turn never substitutes reasoning, and reasoning deltas are accumulated but never streamed as reply text. `ChatOptions::with_reasoning_effort()` sends `reasoning_effort` and is omitted when unset.
