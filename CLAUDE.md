...
- `OpenAIProvider` - OpenAI Chat Completions API (120s timeout, SSE streaming); supports any OpenAI-compatible endpoint via `api_base` (Ollama, Groq, Together, Fireworks, LM Studio, vLLM, DeepSeek, Kimi/Moonshot, Azure OpenAI, Amazon Bedrock, xAI/Grok, Baidu Qianfan); custom auth header via `auth_key_header` field; API version query param via `api_version` field
