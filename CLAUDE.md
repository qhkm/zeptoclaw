...
```bash
cargo fmt && cargo clippy -- -D warnings && cargo test --lib --bin zeptoclaw --test cli_smoke --test integration --test e2e && cargo test --doc && cargo fmt -- --check
```
...
├── channels/    # Telegram, Slack, Discord, Webhook, WhatsApp Web/Cloud, Lark, Email, MQTT, Serial
...
cargo test --bin zeptoclaw                 # Main-binary tests
cargo test --test cli_smoke --test e2e --test integration
