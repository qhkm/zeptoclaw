...
- Browser tool: `browser` wraps `agent-browser` CLI with Lightpanda default (10x faster/lighter than Chrome), automatic Chrome fallback on navigation failure, SSRF pre-/post-navigation validation, and per-session engine sticky switching; supersedes `web_fetch` when enabled
