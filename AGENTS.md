...
- Upstream sync workflow: `.github/workflows/upstream-sync.yml` runs every 4 hours, requires `UPSTREAM_REPOSITORY` (repo variable or manual dispatch input), and only uses Factory Droid auto-resolution when both `FACTORY_API_KEY` and `FACTORY_CLI_SHA256` secrets are set
- Tests: 1613 lib + 54 main + 23 cli_smoke + 68 integration + 140 doc (116 passed, 24 ignored)
